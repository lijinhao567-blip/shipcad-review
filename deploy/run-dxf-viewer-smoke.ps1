...
if (-not $Sample) {
    $Sample = Join-Path $root "datasets\parser\cases\dense_deck_grid.dxf"
}
...
try {
    ...
    $args = @(
        "tools\run_dxf_viewer_webgl_smoke.mjs",
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "DXF viewer smoke failed with exit code $LASTEXITCODE"
    }
} finally {
    try {
