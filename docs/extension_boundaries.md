...
3. 用 MinIO 或其它 S3 兼容服务完成真实容器 E2E、权限配置、bucket 生命周期、备份恢复和大文件性能验收；当前开发机 E2E 已覆盖基础 S3 链路，报告仍保存于数据库，后续可迁移为对象存储附件。
