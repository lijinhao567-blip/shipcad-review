...
- 审查任务队列：默认内存队列必须保持本地测试和 golden E2E 稳定；Redis 协议队列必须验证入队载荷、容量限制、健康检查、处理队列恢复和消费失败路径。`deploy/run-redis-queue-e2e.ps1` 已覆盖真实 Redis 协议开发机 E2E；真实 Valkey 容器 E2E、任务重试和异常恢复演练仍是后续部署验收项。
