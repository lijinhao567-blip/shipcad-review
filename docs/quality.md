...
- Browser WebGL smoke for complex DXF fixtures should verify the product path `backend file endpoint -> frontend Blob URL -> DxfViewerPreview -> WebGL canvas`, including visible layer list, no official-preview failure state, and nonblank WebGL canvas pixels. Run `.\deploy\run-dxf-viewer-smoke.ps1` for the automated local smoke; it writes `.run/dxf-viewer-webgl-smoke.json` and `.run/dxf-viewer-webgl-smoke.png`. This is a release/demo check rather than a Canvas diagnostic fallback.
- Browser issue-focus smoke should verify the product path `ReviewIssue -> ReviewEvidence.location -> DxfViewerPreview focus/highlight`, not only generic drawing load. Run `.\deploy\run-dxf-issue-focus-smoke.ps1`; it uploads the `LAYER_NAME_STANDARD` golden case, creates a real review task, clicks the exact generated issue card through `data-issue-id`, waits for the "已聚焦问题" state, and verifies that red selected-issue pixels increase on the official WebGL canvas. This check proves issue定位闭环, not YOLO/OCR accuracy.
