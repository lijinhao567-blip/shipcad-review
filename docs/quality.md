...
- Golden dataset：`datasets/rules/expected.json` 中每个合成 DXF 样例都要通过 `tools/run_golden_e2e.py`，覆盖合规样例、图层命名、空图层、标题栏、版本号、占位文字和实体数量异常，并校验 `ReviewIssue.layerName` / `ReviewIssue.entityRef` 等定位证据。
